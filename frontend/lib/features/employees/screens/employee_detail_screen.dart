...
class EmployeeDetailScreen extends StatelessWidget {
          ...
          child: Column(
            ...
            children: [
              ...
              _Card(
                children: [
                  _InfoRow(
                    label: tr(context, 'emp_role'),
                    trailing: RoleBadge(role: emp.role),
                  ),
                  _InfoRow(
                    label: tr(context, 'emp_ai_access'),
                    trailing: AiAccessBadge(access: emp.aiAccess),
                  ),
                  ...
                  _InfoRow(
                    ...
                    value: emp.langPref == 'ar'
                        ? tr(context, 'lang_ar')
                        : tr(context, 'lang_en'),
                  ),
                  if (emp.lastActive != null)
                    _InfoRow(
                      label: tr(context, 'emp_last_active'),
                      value: _fmtAgo(context, emp.lastActive!),
                    ),
                ],
              ),
              const SizedBox(height: AppSpacing.xl),
              ...
              _Card(
                children: [
                  ...
                  Wrap(
                    spacing: 6,
                    runSpacing: 6,
                    children: roleDef.permissionKeys
                        .map(
                          (k) => _PermChip(
                            label: tr(context, k),
                ...
                children: [
                  ...
                  if (emp.status == EmpStatus.suspended)
                    _ActionTile(
                      ...
                      onTap: () => empState.reactivate(emp.id),
                    ),
                  if (emp.status == EmpStatus.invited)
                    _ActionTile(
                      icon: Icons.send,
                      label: tr(context, 'emp_resend'),
                      color: AppColors.info,
                      onTap: () => ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(tr(context, 'ux_invite_resent')),
                  ...
                  _ActionTile(
                    ...
                    label: tr(context, 'emp_change_role'),
                    color: AppColors.primary,
                    onTap: () => _showRoleDialog(context, empState, emp),
                  ),
                  _ActionTile(
                    icon: Icons.auto_awesome,
                    label: tr(context, 'emp_change_ai'),
                    color: AppColors.accent,
                    onTap: () => _showAiDialog(context, empState, emp),
                  ),
                ],
              ),
  ...
  String _fmtAgo(BuildContext context, DateTime dt) {
    ...
    if (diff.inMinutes < 60)
      return '${diff.inMinutes} ${tr(context, 'ux_min_ago')}';
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: AppRole.values.where((r) => r != AppRole.owner).map((r) {
            final key = switch (r) {
  ...
  void _showAiDialog(BuildContext context, EmployeesState state, Employee emp) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: AiAccess.values.map((a) {
            final key = switch (a) {
