      ...
      child: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 900),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (state.items.isEmpty)
            Center(child: Padding(padding: const EdgeInsets.all(AppSpacing.xxl), child: Text(tr(context, 'stk_empty'), style: AppTypography.bodyMedium.copyWith(color: AppColors.textSecondary))))
          else
            ...state.items.map((item) => Padding(
              padding: const EdgeInsets.only(bottom: AppSpacing.sm),
              child: InventoryCard(item: item, warehouseName: state.warehouseName(item.warehouseId)),
            )),
