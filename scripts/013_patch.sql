...
DROP POLICY IF EXISTS ws_campaigns ON campaigns;
CREATE POLICY ws_campaigns ON campaigns
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE segments ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_segments ON segments;
CREATE POLICY ws_segments ON segments
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE loyalty_programs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_loyalty_programs ON loyalty_programs;
CREATE POLICY ws_loyalty_programs ON loyalty_programs
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE loyalty_accounts ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_loyalty_accounts ON loyalty_accounts;
CREATE POLICY ws_loyalty_accounts ON loyalty_accounts
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE referral_programs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_referral_programs ON referral_programs;
CREATE POLICY ws_referral_programs ON referral_programs
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE referrals ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_referrals ON referrals;
CREATE POLICY ws_referrals ON referrals
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE nurturing_sequences ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_nurturing_sequences ON nurturing_sequences;
...
DROP POLICY IF EXISTS ws_drivers ON drivers;
CREATE POLICY ws_drivers ON drivers
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE delivery_zones ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_delivery_zones ON delivery_zones;
CREATE POLICY ws_delivery_zones ON delivery_zones
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE delivery_assignments ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_delivery_assignments ON delivery_assignments;
CREATE POLICY ws_delivery_assignments ON delivery_assignments
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE cod_collections ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_cod_collections ON cod_collections;
...
DROP POLICY IF EXISTS ws_workspace_country_packs ON workspace_country_packs;
CREATE POLICY ws_workspace_country_packs ON workspace_country_packs
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE tax_rules ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_tax_rules ON tax_rules;
CREATE POLICY ws_tax_rules ON tax_rules
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE retention_policies ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_retention_policies ON retention_policies;
CREATE POLICY ws_retention_policies ON retention_policies
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE archival_jobs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_archival_jobs ON archival_jobs;
...
DROP POLICY IF EXISTS ws_brand_kits ON brand_kits;
CREATE POLICY ws_brand_kits ON brand_kits
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE media_assets ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_media_assets ON media_assets;
CREATE POLICY ws_media_assets ON media_assets
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE media_generation_requests ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_media_generation_requests ON media_generation_requests;
...
DROP POLICY IF EXISTS ws_workspace_integrations ON workspace_integrations;
CREATE POLICY ws_workspace_integrations ON workspace_integrations
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE webhook_subscriptions ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_webhook_subscriptions ON webhook_subscriptions;
CREATE POLICY ws_webhook_subscriptions ON webhook_subscriptions
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE import_jobs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_import_jobs ON import_jobs;
CREATE POLICY ws_import_jobs ON import_jobs
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE export_jobs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_export_jobs ON export_jobs;
CREATE POLICY ws_export_jobs ON export_jobs
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID)
    WITH CHECK (workspace_id = current_setting('app.workspace_id', true)::UUID);

ALTER TABLE sync_logs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_sync_logs ON sync_logs;
