...
DROP POLICY IF EXISTS ws_orders ON orders;
CREATE POLICY ws_orders ON orders
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1B: invoices
ALTER TABLE invoices ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_invoices ON invoices;
CREATE POLICY ws_invoices ON invoices
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1C: payments
ALTER TABLE payments ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_payments ON payments;
CREATE POLICY ws_payments ON payments
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1D: contacts
ALTER TABLE contacts ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_contacts ON contacts;
CREATE POLICY ws_contacts ON contacts
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1E: products
ALTER TABLE products ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_products ON products;
CREATE POLICY ws_products ON products
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1F: accounts (chart of accounts)
ALTER TABLE accounts ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_accounts ON accounts;
CREATE POLICY ws_accounts ON accounts
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1G: journal_entries
ALTER TABLE journal_entries ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_journal_entries ON journal_entries;
CREATE POLICY ws_journal_entries ON journal_entries
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1H: transactions
ALTER TABLE transactions ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_transactions ON transactions;
CREATE POLICY ws_transactions ON transactions
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1I: attendance
ALTER TABLE attendance ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_attendance ON attendance;
CREATE POLICY ws_attendance ON attendance
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1J: payroll
ALTER TABLE payroll ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_payroll ON payroll;
CREATE POLICY ws_payroll ON payroll
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1K: shipments
ALTER TABLE shipments ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_shipments ON shipments;
CREATE POLICY ws_shipments ON shipments
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1L: fixed_assets
ALTER TABLE fixed_assets ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_fixed_assets ON fixed_assets;
CREATE POLICY ws_fixed_assets ON fixed_assets
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1M: recurring_expenses
ALTER TABLE recurring_expenses ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_recurring_expenses ON recurring_expenses;
CREATE POLICY ws_recurring_expenses ON recurring_expenses
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1N: warehouses
ALTER TABLE warehouses ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_warehouses ON warehouses;
CREATE POLICY ws_warehouses ON warehouses
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1O: departments
ALTER TABLE departments ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_departments ON departments;
CREATE POLICY ws_departments ON departments
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1P: branches
ALTER TABLE branches ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_branches ON branches;
CREATE POLICY ws_branches ON branches
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1Q: roles
ALTER TABLE roles ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_roles ON roles;
CREATE POLICY ws_roles ON roles
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1R: shifts
ALTER TABLE shifts ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_shifts ON shifts;
CREATE POLICY ws_shifts ON shifts
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1S: notifications
ALTER TABLE notifications ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_notifications ON notifications;
CREATE POLICY ws_notifications ON notifications
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1T: audit_logs
ALTER TABLE audit_logs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_audit_logs ON audit_logs;
CREATE POLICY ws_audit_logs ON audit_logs
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1U: document_sequences
ALTER TABLE document_sequences ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_document_sequences ON document_sequences;
CREATE POLICY ws_document_sequences ON document_sequences
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1V: product_categories
ALTER TABLE product_categories ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_product_categories ON product_categories;
CREATE POLICY ws_product_categories ON product_categories
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1W: taxes
ALTER TABLE taxes ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_taxes ON taxes;
CREATE POLICY ws_taxes ON taxes
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1X: bill_of_materials
ALTER TABLE bill_of_materials ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_bill_of_materials ON bill_of_materials;
CREATE POLICY ws_bill_of_materials ON bill_of_materials
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1Y: production_orders
ALTER TABLE production_orders ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_production_orders ON production_orders;
CREATE POLICY ws_production_orders ON production_orders
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1Z: projects
ALTER TABLE projects ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_projects ON projects;
CREATE POLICY ws_projects ON projects
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1AA: tasks
ALTER TABLE tasks ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_tasks ON tasks;
CREATE POLICY ws_tasks ON tasks
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1AB: bookings
ALTER TABLE bookings ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_bookings ON bookings;
CREATE POLICY ws_bookings ON bookings
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1AC: pos_terminals
ALTER TABLE pos_terminals ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_pos_terminals ON pos_terminals;
CREATE POLICY ws_pos_terminals ON pos_terminals
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1AD: pos_sessions
ALTER TABLE pos_sessions ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_pos_sessions ON pos_sessions;
CREATE POLICY ws_pos_sessions ON pos_sessions
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1AE: attachments
ALTER TABLE attachments ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_attachments ON attachments;
CREATE POLICY ws_attachments ON attachments
    USING (workspace_id = current_setting('app.workspace_id', true)::UUID);

-- 1AF: inventory_levels (workspace_id was backfilled in 007, NOT NULL in 008)
ALTER TABLE inventory_levels ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ws_inventory_levels ON inventory_levels;
...
-- Query: SELECT * FROM user_permission_overrides WHERE membership_id = ? AND expires_at IS NULL
...
COMMENT ON INDEX idx_overrides_membership_active IS
    'RBAC: fast lookup of active (non-expired) permission overrides for conflict resolution.';


-- 2D: Active permission delegations
-- Query: SELECT * FROM permission_delegations WHERE delegate_membership_id = ? AND status = 'active'
...
-- SKIPPED: workspaces table does not have a slug column in the current schema.
-- Uncomment when slug column is added.
-- DO $$
-- BEGIN
--     IF NOT EXISTS (
--         SELECT 1 FROM pg_constraint
