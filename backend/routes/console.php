...
$schedulerMode = (string) config('operations.scheduler_mode', 'application');

if (in_array($schedulerMode, ['all', 'application'], true)) {
    // Operational heartbeats. These verify that the application scheduler is
    // ticking and that a real queue worker is consuming jobs.
    ...
    // AI Advisor enters a tenant runtime connection for each workspace.
...
if (in_array($schedulerMode, ['all', 'backup'], true)) {
    // Backup tasks run in a separate container which receives only the
    // dedicated read-only pg_dump identity, never the application identities.
